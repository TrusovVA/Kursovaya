...
axes(handles.axes1); plot(f1,a1(1:length(f1))); grid; xlim([0 50]);
...
a2=abs(y2)/N2;
a4=real(a2);
axes(handles.axes3); plot(f1,a4(1:length(f1))); grid; xlim([0 50]);
title('Спектр сигнала испорченного белым шумом');
xlabel('Частота дискретизации');
ylabel('Амплитуда');
a22=smooth(f1,a4(1:length(f1)),0.002);
axes(handles.axes4); plot(f1,a22(1:length(f1))); grid; xlim([0 50]);
